use softcomp;
DROP PROCEDURE IF EXISTS set_bs_guess_tag;
DELIMITER $
CREATE PROCEDURE set_bs_guess_tag()
  LANGUAGE SQL
  DETERMINISTIC
  MODIFIES SQL DATA
...
BEGIN
  ...
  DECLARE my_cursor CURSOR FOR ( SELECT distinct prod_inst_num_first_enc FROM sel_bs_guess ORDER BY prod_inst_num_first_enc );
  DECLARE sig_bs_cur CURSOR FOR ( SELECT start_time,base_station,sector FROM sel_bs_guess WHERE prod_inst_num_first_enc=v_mdn_no ORDER BY start_time );
  ...
  myloop_tag:LOOP
    ...
    BEGIN
      ...
      insert into sel_bs_guess_tag(mdn,wday_out_time,wday_in_time,eday_out_time,eday_in_time,wd_move_loc,we_move_loc)
