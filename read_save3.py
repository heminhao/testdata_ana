...
db_meta = sy.MetaData()
db_tab_sel_userflow = sy.Table('hive_bs_info', db_meta, autoload=True, autoload_with=db_engine)
ins_stat = db_tab_sel_userflow.insert()
print('db conn ok')

eachfile = 'bs_info_split.csv'
print(read_dir + eachfile + ' file begin dumping...')
f = io.open(read_dir + eachfile, 'r', encoding='utf8')
field_names = ['bs_seq', 'sector', 'bsid', 'longitude', 'latitude']
